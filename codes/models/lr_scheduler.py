...
class ExponentialLR(_LRScheduler):
    def __init__(self, optimizer, decay_step=0.1, num_epoch=30,last_epoch=-1):
        self.decay_step = decay_step
        self.num_epoch = num_epoch
        self.little_mul = decay_step**(1.0/num_epoch)
        super(ExponentialLR, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        current_epoch = self.last_epoch
        current_mul = self.little_mul**current_epoch
                
        return [group['initial_lr']*current_mul for group in self.optimizer.param_groups]
