...
class ModulatedDeformConvPack(ModulatedDeformConv):
    ...
    def forward(self, x):
        if self.extra_offset_mask:
            ...
            out = self.conv_offset_mask(x)   ## Just one conv
        # out: B * (groups*3*kernel_size*kernel_size) * H * W
        # group = 8
        # o1, o2, mask: B * (groups*kernel_size*kernel_size) * H * W
        o1, o2, mask = torch.chunk(out, 3, dim=1)
        offset = torch.cat((o1, o2), dim=1)  # B,2,group*kernel*kernel,H,W
        ...
        # x: B * CH * H * W
        # offset: B * (groups*kernel_size*kernel_size) * H * W
        # self.weight, self.bias, self.groups not find ????
        return modulated_deform_conv(x, offset, mask, self.weight, self.bias, self.stride,
                                     self.padding, self.dilation, self.groups,
                                     self.deformable_groups)
